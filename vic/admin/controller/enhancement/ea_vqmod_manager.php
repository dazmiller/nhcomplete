...
class ControllerEnhancementEaVqmodManager extends Controller {
	...
	public function index() {
		...
		$this->document->addStyle('view/template/enhancement/css/bootstrap-radio.css');
	...
	public function getList() {
		...
		if ($this->config->get('ea_tools_notifications_enable')) {
			$data['ea_tools_notifications_enable'] = $this->config->get('ea_tools_notifications_enable');
		} else {
			$data['ea_tools_notifications_enable'] = '0';
		}
	...
	protected function getForm($vqmod_script) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => html_entity_decode($this->url->link('enhancement/ea_vqmod_manager/edit_vqmod_script', 'token=' . $this->session->data['token'] . '&vqmod=' . $vqmod_script, true))
		);
